"""Web adapters for Goldman Sachs written insights.

Both the corporate site (gs.com) and GSAM (am.gs.com) are AEM/Next.js sites that
...
paths (gs.com disallows /insights/top-of-mind/, /what-we-do/research/, etc.).
"""
...
def _fetch_article(client, url, canonical, item_id, source: Source, lastmod=None) -> Item | None:
    ...
    if not title:
        return None
    title = re.split(r"\s+[|I]\s+Goldman Sachs", title)[0].strip() or title
