"""Web adapters for BlackRock written insights.

The corporate site (blackrock.com, incl. the BlackRock Investment Institute) is
an AEM/Next.js site that renders article lists client-side but publishes public
**sitemaps with <lastmod>**. So — exactly like the JPM/GS corporate-hub pattern —
we read the sitemap for the full URL list (no headless browser), filter by path,
and pull each article's title/summary from og: metadata. One generic,
config-driven adapter (`sitemap_articles`) serves every such source; per-source
`params` in sources.yaml supply the sitemap URL and the include/exclude filters.

robots.txt is respected: each source's `params.exclude` lists the disallowed
paths. NOTE: www.blackrock.com sits behind aggressive Akamai bot protection
(returns 403 to automated requests); this adapter sends a browser-like UA but
does NOT attempt to bypass any gating — per-source failures are isolated and a
403 simply yields no items for that source (see sources.yaml note).
"""
