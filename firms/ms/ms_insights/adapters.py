"""Web adapters for Morgan Stanley written insights.

Both the corporate site (morganstanley.com) and MSIM (morganstanley.com/im) are AEM/Next.js sites that
render article lists client-side but publish public **sitemaps with <lastmod>**.
...
paths (morganstanley.com disallows /insights/top-of-mind/, /what-we-do/research/, etc.).
"""
...
def _fetch_article(client, url, canonical, item_id, source: Source, lastmod=None) -> Item | None:
    ...
    if not title:
        return None
    title = re.split(r"\s+[|I]\s+Morgan Stanley", title)[0].strip() or title
